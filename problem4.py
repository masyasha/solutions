...
mass = """ kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
matches = re.findall("[a-z][A-Z][A-Z][A-Z][a-z][A-Z][A-Z][A-Z][a-z]", mass)

rawAnswer = re.findall("[a-z]", (" ".join(matches)))

print rawAnswer  # the thing here is that it finds combination of one small,
                 # three big, one small, three big and a small letters. but we 
                 # only need the small letter in the center so keep it in mind.
                 # i ran out patience to write a script that would print only them,
                 # so the key letter is inside of 2 other small letters around it.
